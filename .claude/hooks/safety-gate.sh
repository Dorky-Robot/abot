#!/usr/bin/env bash
# PreToolUse safety gate hook (deny-list-only)
#
...
#   SAFETY_GATE_AUDIT_LOG — path to audit log file (NDJSON format); unset = no logging
#
...
audit_log() {
	...
	if [[ -n "${SAFETY_GATE_AUDIT_LOG:-}" ]]; then
		local timestamp
		...
		jq -cn \
			...
			>>"$SAFETY_GATE_AUDIT_LOG" 2>/dev/null || true
	fi
}
...
# Check whether a path is within any safe directory (project, ~/.claude).
is_within_safe_dir() {
	local path="$1"
